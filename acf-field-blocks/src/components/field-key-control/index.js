...
const FieldKeyControl = ({
	...
	useEffect( () => {
		...
		if ( 'current_post' === source ) {
				...
				if (siteTemplate.includes('//')) {
					...
					if ( templateSlug ) {
						...
						if ( templateSlug.startsWith('single-') ) {
							postType = templateSlug.replace(/^single-/,'');
			...
			if ( isDescendentOfTermLoop ) {
				setFieldSource({
					object: 'term',
					type: context.taxonomy
				})
			} else if ( context?.["acf-field-blocks/taxonomy"] ) { // backwards compatibility
				setFieldSource({
					object: 'term',
					type: context["acf-field-blocks/taxonomy"]
				})
			} else if ( isSiteEditor ) {
				const siteTemplate = select( 'core/editor' ).getCurrentPostId();
				let templateSlug = '';
				[ , templateSlug ] = siteTemplate.split('//');
				if ( templateSlug ) {
					let taxonomy = false;
	...
	if ( isLoadingOptions ) {
		return (
			<BaseControl __nextHasNoMarginBottom={true} label={ label }>
				<div>
					<Spinner />
				</div>
			</BaseControl>
